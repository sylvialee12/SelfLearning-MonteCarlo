...
class slmc(wolff.wolff):
    ...
    def __init__(self,temperature,Hamiltonian,init_sample,Hreal):
        ...
        """
        super().__init__(temperature,Hamiltonian,init_sample,nsteps=1000)
        self.Hreal=Hreal
...
def main(J,K,Nx,Ny,temperature,mcsetN,Jeff=None):
    ...
    if Jeff==None:
        Jlist,effetiveH=GetEffective.train(J,K,Nx,Ny,temperature+4,temperature,mcsetN=mcsetN)
    else:
    ...
    spin_chain2=Hreal.spin_chain_generation(temperature,initsampleset,k=2)
...
def visulizedata():
    ...
    plt.figure("Energy")
    plt.plot(energy1,"b--",label="SLMC")
    plt.plot(energy2,"g-",label="Local update")
    plt.legend(loc='upper right', shadow=True)
    plt.savefig("data/energy.png")
    plt.figure("AutoCorrelation")
    plt.plot(tau_correlation1,"b--",label="SLMC")
    plt.plot(tau_correlation2,"g-",label="Local update")
    plt.legend(loc='upper right', shadow=True)
